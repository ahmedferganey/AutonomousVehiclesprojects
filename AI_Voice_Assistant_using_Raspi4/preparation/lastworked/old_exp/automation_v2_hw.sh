...
# Automatically detect the SD card device
...
if [ -z "$SD_IMAGE" ]; then
    echo "Error: SD card device not found. Please ensure it is connected and try again."
    exit 1
fi
...
sudo mkfs.vfat -F 32 -n boot ${SD_IMAGE}1
check_error "formatting boot partition"
sudo mkfs.ext4 -L root ${SD_IMAGE}2
check_error "formatting root partition"

# Mount the partitions
echo "Mounting partitions..."
sudo mount ${SD_IMAGE}1 $BOOT_PART
check_error "mounting boot partition"
sudo mount ${SD_IMAGE}2 $ROOTFS_PART
...
check_error "unmounting boot partition"
sudo umount $ROOTFS_PART
check_error "unmounting root partition"
